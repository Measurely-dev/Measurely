...
const RealtimeUpdate = ({
  ...
  return (
    <div
      className={cn(
        "relative flex h-[350px] -mt-20 w-full max-w-[500px] flex-col items-center",
        className
      )}
    >
      {/* SVG Paths */}
      ...
      >
        ...
        {/* Buttons */}
        <g className="stroke-border" fill="none" strokeWidth="0.4">
          {/* First Button */}
          <g>
            <rect
              className="fill-accent dark:fill-[#18181B]"
              x="14"
              y="5"
              width="34"
              height="10"
            ...
            <text
              ...
              fontWeight="500"
            >
              {badgeTexts?.first || "User 1"}
            </text>
          </g>
          {/* Second Button */}
          <g>
            <rect
              className="fill-background dark:fill-[#18181B]"
              x="60"
              y="5"
              width="34"
              height="10"
      ...
      <div className="absolute bottom-10 flex w-full flex-col items-center">
        ...
        {/* box title */}
        <div className="absolute -top-3 z-20 flex items-center justify-center rounded-lg border bg-accent dark:bg-card px-2 py-1 sm:-top-4 sm:py-1.5">
          <SparklesIcon className="size-3" />
          <span className="ml-2 text-[10px]">
            {title ? title : "Realtime metrics updates via API"}
          </span>
        </div>
        {/* box outter circle */}
        <div className="absolute -bottom-8 z-30 grid h-[60px] w-[60px] place-items-center rounded-full border-t bg-accent dark:bg-card font-semibold text-xs">
          <LogoSvg className="w-8 h-8 dark:invert" />
        </div>
        {/* box content */}
        <div className="relative z-10 flex h-[150px] w-full items-center justify-center overflow-hidden rounded-lg border bg-background shadow-md">
          {/* Badges */}
          <div className="absolute bottom-8 left-12 z-10 h-7 rounded-full bg-accent dark:bg-card px-3 text-xs border flex items-center gap-2 ">
            <FileIcon className="size-4" />
            <span>{buttonTexts?.first || "New certificate"}</span>
          </div>
          <div className="absolute right-16 z-10 hidden h-7 rounded-full bg-accent dark:bg-card px-3 text-xs sm:flex border items-center gap-2">
            <User className="size-4" />
            <span>{buttonTexts?.second || "New user"}</span>
          </div>
          {/* Circles */}
          <motion.div
